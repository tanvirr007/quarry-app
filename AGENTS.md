...
- **Storage Volume Discovery & Removable Media**: `StorageVolumeManager` discovers mounted internal and external volumes (SD cards, MicroSD, USB flash drives / pendrives, USB OTG) using multi-probe path resolution (`/storage/<UUID>` FUSE mount first, then `getExternalFilesDirs()` walk-up, fallback to raw mount) and resilient `StatFs` + `File.totalSpace`/`usableSpace` storage calculation. Unmounted or disconnected external volumes are filtered out immediately. Real-time media connect/disconnect observation is provided via `observeVolumeChanges()` (combining `StorageVolumeCallback`, media broadcast receivers, and USB device attach/detach receivers), automatically falling back to primary internal storage when a selected drive is unplugged. Volumes are classified into `StorageDeviceType` (`INTERNAL`, `SD_CARD`, `USB_OTG`).
- **Volume Data Isolation**: Room database (`FileEntity.volumeId`, indices on `volumeId` and `[volumeId, isDirectory]`) isolates files, directories, snapshots, and duplicate candidate queries per volume. Scans and cleanups on an external volume never mix with internal storage.
- **Home Storage Overview Selector**: The volume selector is housed exclusively in the top-right of the Home screen `StorageOverviewCard`. When only one volume is mounted, it displays the standard `x% used >` chip. When multiple volumes are detected, it smoothly transitions via `AnimatedContent` to an interactive Material 3 volume selector pill with dropdown drive selection, device type badges, and haptics.
- **Historical Scan Delta Scoping**: Growth delta (`X KB freed/added since last scan`) is recorded per-volume (`ScanSnapshot.volumePath`) and strictly compares consecutive scans of that specific volume. Drives with fewer than 2 recorded scans cleanly omit the delta badge.
- **External Drive Category Adjustment**: External removable drives do not contain installed applications; `appsSize` is 0L and `StorageCategory.APPS` is automatically excluded from category breakdowns.
- **Explore & Cleanup Scoping**: `ExploreViewModel` and `CleanupViewModel` reactively observe DataStore `selected_volume_id` to dynamically adapt the root browsing boundary (`rootPath`), breadcrumbs, candidate groups, and duplicate checks.
