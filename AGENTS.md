...
- **Historical Scan Delta Scoping**: Growth delta (`+X KB since last scan`) is recorded per-volume (`ScanSnapshot.volumePath`) and strictly compares consecutive scans of that specific volume. Drives with fewer than 2 recorded scans cleanly omit the delta badge.
